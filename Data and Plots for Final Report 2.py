...
NodeTime = []
t = tqdm(range(100),desc = 'Testing Make-Nodes D* Grid = (1, 1)')
for i in t:
    t1 = time.time()
    nodes = makenodes.makenodes(i+1,i+1,4,0,0,0)
    t1 = time.time()-t1
    NodeTime.append(t1)
    t.set_description('Testing Make-Nodes D* Grid = (%s, %s)'%(i+1,i+1))
...
print('\n')
AstarTimes = []
t = tqdm(range(100),desc = 'Testing A* Initial Paths Grid = (1,1)')
for i in t:
    t1 = time.time()
    path = astar(np.zeros((i+1,i+1)),(0,0),(i,i))
    t1 = time.time()-t1
    AstarTimes.append(t1)
...
D1 = np.sum(repairsDstar, axis = 0)/len(repairsDstar)
D2 = np.sum(replansAstar, axis = 0)/len(replansAstar)
disttogoalDstar = disttogoalDstar[0]
disttogoalAstar = disttogoalAstar[0]
print('\n')

# In[ ]:


p1 = plt.scatter(disttogoalDstar,D1,1,label='Test Data')
p2, = plt.plot(disttogoalDstar,best_fit(disttogoalDstar,D1,2),label = '2nd Order PloyFit',color = 'red')
...
p2, = plt.plot(best_fit(disttogoalDstar,D1,2),label = '2nd Order PloyFit',color = 'red')
...
p1 = plt.scatter(disttogoalAstar,D2,2,label = 'Test Data')
p2, = plt.plot(disttogoalAstar,best_fit(disttogoalAstar,D2,2), label = '2nd Order PolyFit',color = 'red')
